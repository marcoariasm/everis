...
/**
   * Returns list of bulleted items
	 ...
	 * @return {ListUl}
	 */
const BulletedList2 = ({ textList, color = allColors.colorOrangeMain }) => {
  //console.log(textList[5].name);
    return (
        <ListUl color={color} className="bodyText">
            <ul>
              {textList.map((text, i) => <li key={i}><Link to={{
                pathname: `/generic-procedures/with-beneficiaries/`,
                search: text.link,
              }
                  }><span>{text.name}</span></Link></li>)}
              </ul>
        </ListUl>
    )
}
