  ...
  return (
    <Wrapper show={show} ref={refModal}>
      ...
      >
        ...
        >
          {!hideButtonCancel && !hidden && (
            <Button size="18" classButton="btn-actualizar" widthB={widthB} heightB={heightB}>
              {label}
            </Button>
          )}
          {hideButtonCancel && !hidden && (
            <Button size="18" widthB={widthB} heightB={heightB} classButton="btn-pagina-principal" onClick={onClick}>
              {nameButton}
            </Button>
          )}
        </Footer>
...
const Content = styled.div`
  ...
  height: ${({ heightmobile }) => heightmobile + 'px' || 'auto'};
  padding: 10px;
  border-radius: 15px;
  ...
  @media only screen and (min-width: ${size.tablet}) and (max-width: ${size.laptopL}) {
    ...
    max-height: ${({ heightTablet }) => `${heightTablet}px` || 'auto'};
  }
