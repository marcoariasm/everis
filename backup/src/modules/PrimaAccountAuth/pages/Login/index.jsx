...
import { isEmpty, ifElse, propEq, always, prop } from 'ramda'
...
const Login = () => {
  ...
  const handleAuth = async () => {
    ...
    if (!password || !document) {
      ...
      dispatchAuth(token, documentTypeSelected, document, password)
  ...
  const dispatchAuth = async (token, documentTypeSelected, document, password) => {
    ...
    try {
      ...
      dispatch(UI_SetError(errorMessage))
      dispatch(UI_StopLoading())
    }
  }
        ...
        <ContentForm>
          <ContentDataForm>
            ...
            <AffiliateForm>
              ...
              {storeUI.loading ? (
                <img src={loading} alt="Loading" style={{ height: '7rem', margin: ' 0 auto', display: 'block' }} />
              ) : (
                <RecoverAccountSection>
                  <div>
