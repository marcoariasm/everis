...
import { filter, isEmpty, or } from 'ramda';
...
function RegisterBeneficiaries({ setStep, step }) {
	...
	const [benefSelected, setBenefSelected] = useState([]);
	...
	function getDocumentSelected(event) {
		if (event.checked) {
			setBenefSelected(benefSelected.push(parseInt(event.value)));
		} else {
			setBenefSelected(filter((docBeneficiaries) => docBeneficiaries !== event.value, benefSelected));
		}
		dispatch(setSelectedBeneficiares(benefSelected));
	}
