...
const FormNewBeneficiary = ({
	...
	const [
		...
		ValidSexOfConcubine,
	] = useValidationBeneficiaries(affiliateInfo, infoIni, store);
	...
	const handleKeyUp = (e) => {
		...
		if (datos.documentType === prop('type', typeDocument[1])) {
	...
	const handleOnBlur = (e) => {
		...
		if (name === 'documentNumber') {
			...
			if (datos.documentType === prop('type', typeDocument[0])) {
				if (value.length !== 8) {
		...
		if (name === 'birthdate') {
			...
			if (!isCorrect) {
				setValidation({ birthdate: 'La fecha de nacimiento es incorrecta.' });
				setCustomValidation2('La fecha de nacimiento es incorrecta.');
			} else {
				setValidation({});
				setCustomValidation2(null);
			}
