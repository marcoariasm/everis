...
const CardUploadFile = ({ businessDay, email, onChange }) => {
  ...
  const onDrop = useCallback((acceptedFiles) => {
    ...
    setTimeout(() => {
      setLoading(false)
      serviceFetcher('http://52.177.29.171:8080/document-manager/upload', {
        method: 'POST',
        body: formData,
      }).then((response) => {
          ...
          <Content direction="row">
            <CardInformationGray
              ...
              text={`Este documento será validado con SUNAT, de no estar conforme el ${moment(businessDay).format(
                'DD/MM/YYYY'
              )} te lo notificaremos a tu correo electrónico ${toLower(
                email
              )}. En ese caso, no podrás continuar con el trámite.`}
            />
          </Content>
          <ContentFileName>
            <div>
